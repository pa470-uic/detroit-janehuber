...
# %>% 
#   mutate(sale_date = factor(sale_date, levels=years_for_evaluation),
#          sale_date = as.numeric(sale_date))
...
sales_and_assessment_differences_graph <- 
  ...
  xlab("Year") + 
  ylab("Percent") + 
  ggtitle("Difference in Dollars Between of Assessments Higher Than Sales in Detroit") +
  custom_theme
...
```{r cmfproperty-sales-ratio, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}
...
# lm(sale.price ~ gross.square.feet + age + sale.year, data = sale_df)
