...
blight_clean <- 
  ...
  select(c(parcel_num, violation_date))
...
# Foreclosures (Filter out old data)

years_for_evaluation <- c('2011', '2012', '2013', '2014', '2015', '2016', '2017', '2018', '2019')
...
assessments_clean %>% 
  filter(assessment_year %in% c(2017, 2018, 2019, 2020, 2021, 2022),
         assessed_value < (sd(assessed_value) * 2),
         assessed_value > 0) %>% 
  mutate(assessment_year = as.factor(assessment_year)) %>% 
  ggplot() +
...
# Ideal: can I have a dot of all sales from 2012-2020
sales_trends_over_time <- 
  sales_clean %>% 
  filter(sale_price < (sd(sale_price) * 3)) %>% 
  ggplot() +
  geom_point(aes(x=sale_date, y = sale_price))

```


Similarly, the Harris School of Public Policy at the University of Chicago's The Center For Municipal Finance produced the report ["An Evaluation of Residential Property Tax Assessments in the City of Detroit, 2016-2018"](https://harris.uchicago.edu/files/evalrespropertytaxasdetroit20162018.pdf) that looks at similar trends across home assessments in Detroit. 
