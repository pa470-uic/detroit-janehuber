...
sale_price_recipe <- 
  recipe(sale_price ~ ward + zip_code + sale_price + total_square_footage + assessed_value,
      data = train_sales) %>%
  step_naomit(ward) %>% 
  step_naomit(zip_code) %>% 
  step_naomit(sale_price) %>% 
  step_naomit(total_square_footage) %>% 
  step_naomit(assessed_value) %>% 
    step_other(ward, threshold = 0.01) %>% 
  step_other(zip_code, threshold = 0.01) %>% 
  step_unknown(all_nominal_predictors()) %>%
  step_dummy(all_nominal_predictors())
...
sale_price_recipe_model_2 <- 
  ...
  step_naomit(zip_code) %>% 
  step_naomit(assessed_value) %>% 
  step_naomit(percent_zip_foreclosures) %>% 
  step_naomit(avg_sale_price_zip) %>% 
  step_other(ward, threshold = 0.01) %>% 
  step_other(zip_code, threshold = 0.01) %>% 
  step_unknown(all_nominal_predictors()) %>%
  step_dummy(all_nominal_predictors())
...
best_results_fit <- 
   ...
   last_fit(split = your_testing_data)
