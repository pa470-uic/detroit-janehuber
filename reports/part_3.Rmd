...
---
Data Setup TODO:
(x) Add in latitude and longitude so that we have this info for later
() Get the census tract data integrated.

Part A TODO (max 30 minutes on Sunday--4 pm!):
...
Part B TODO:
(x) Need to create a second predictor. (maybe "Sale price per square foot for neighborhood")
(x) Create a table for my results of the model, compare the two models... Need a format for this.
(x) Follow this for my second model (sale price model).
(x) Set up comparison between first and second sale price model.
() BONUS: Instead of printing the tibble, use Kable to display the results.

Part C TODO: 
(x) out-of-sample for overassessment in 2016
(x) out-of-sample for sales in 2019
() BONUS: commentary on these models, what we can learn, maybe a graph/comparison

Part D TODO:
() Data prep to get census tract for XY variables
...
sale_price_recipe <- 
  recipe(sale_price ~ ward + sale_price + zip_code + total_square_footage + assessed_value,
      data = train_sales) %>%
  step_other(ward, threshold = 0.01) %>% 
  step_other(zip_code, threshold = 0.01) %>% 
  step_unknown(all_nominal_predictors()) %>%
...
Second model is improved; still a high RMSE.
...
sale_price_predictions_census_tract <- 
  sale_price_predictions %>% 
  filter(!is.na(X),
         !is.na(Y)) %>% 
...
sale_preds_grouped_census <- 
  sale_price_predictions_census_tract %>% 
  group_by(GEOID) %>% 
  summarise(average_predictions = mean(.pred, rm.na=TRUE),
            geometry = st_combine(geometry)) 


# Seeing if I need to convert this multipolygon into a polygon
# %>% 
#   mutate(geometry = sf_)
...
#doesn't work
leaf_sf_col(sale_preds_grouped_census$geometry, sale_preds_grouped_census$average_predictions,
     col_method = "bin", 
...
leaflet() %>% 
  ...
  addPolygons(data = sale_preds_grouped_census)

#Doesn't work
base_map <- 
  sale_preds_grouped_census %>% 
  leaflet(geometric_points) %>%
  addProviderTiles(provider = "CartoDB.Positron") %>% 
  addPolygons(
      map=geometry,
      fillColor = ~pal(average_predictions),
                stroke = FALSE,
                smoothFactor = 0,
                fillOpacity = 0.7) %>%
    addLegend("bottomright", 
              pal = pal, 
              values = ~ average_predictions,
