...
Part D TODO:
() Data prep to get census tract for XY variables--just need to convert into a polygon
...
sale_price_recipe <- 
  recipe(sale_price ~ ward + zip_code + total_square_footage + assessed_value,
      data = train_sales) %>%
  step_other(ward, threshold = 0.01) %>% 
  step_other(zip_code, threshold = 0.01) %>% 
  step_unknown(all_nominal_predictors()) %>%
...
#Can I get the most common pred class per GEOID?
sale_preds_grouped_census <- 
  overassessment_predictions_census_tract %>% 
  ount(GEOID, overassessed, .pred_class)  %>% 
  group_by(GEOID) %>% 
  summarise(average_predictions = ),
