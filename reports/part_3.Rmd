...
train_model_2 <- training(split)
test_model_2 <- testing(split)

# Add new field here 
overassessed_second_recipe <- 
  recipe(overassessed ~ percent_zip_foreclosures + sale_price + zip_code,
      data = train_model_2) %>%
  step_other(ward, threshold = 0.01) %>% 
  step_other(zip_code, threshold = 0.01) %>% 
  step_unknown(all_nominal_predictors()) %>%
...
overassessed_workflow <- 
  overassessed_workflow %>% 
  remove_recipe(overassessed_first_recipe) %>% 
